...
//        // Đặt lịch chạy task với delay tính bằng milli giây, lặp lại sau mỗi khoảng thời gian (tính từ lúc bắt đầu task trước đến lúc bắt đầu task tiếp theo)
        ScheduledFuture scheduledFuture2 = executor.scheduleAtFixedRate(scheduledRequestHandler, 2000, 500, TimeUnit.MILLISECONDS);
        while (!scheduledFuture2.isDone()) {
            if (scheduledRequestHandler.i.get() > 40) {
                scheduledFuture2.cancel(false); // Hủy chạy task
            }
        }
        // Đặt lịch chạy task với delay tính bằng milli giây, lặp lại sau mỗi khoảng thời gian (tính từ lúc kết thúc task trước đến lúc bắt đầu task tiếp theo)
        ScheduledFuture scheduledFuture3 = executor.scheduleWithFixedDelay(scheduledRequestHandler, 2000, 500, TimeUnit.MILLISECONDS);
